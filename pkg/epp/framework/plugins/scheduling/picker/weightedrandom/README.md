...
2.  If all candidates have a score of zero or less, it delegates to the [`random-picker`](../random/README.md) for uniform selection.
...
This picker resolves the trade-off between [`max-score-picker`](../maxscore/README.md) and [`random-picker`](../random/README.md). It prefers higher-scoring endpoints while maintaining exploration and avoiding extreme hot-spotting. It is ideal when paired with scorers that produce continuous scores (e.g., Token Load or Latency).
