...
if [ "${EPD_ENABLED}" == "\"true\"" ] && [ ${VLLM_DATA_PARALLEL_SIZE} -ne 1 ]; then
  # E-PD + Data Parallel is not supported yet
  echo "Error: VLLM_DATA_PARALLEL_SIZE cannot be set when EPD_ENABLED is true."
  exit 1
fi
